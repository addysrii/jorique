...
import { productService } from '../lib/api/products'; // Import your product service
...
export default function AdminDashboard() {
  ...
  // Calculate real stats based on products
  ...
  const realStats = data?.stats?.length ? data.stats : [
    ...
    { label: 'Revenue', value: 'N/A' }, // Customize later if you have sales data
  ];
              ...
              <section className="bg-white border border-border rounded-lg p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {products.slice(0, 10).map((product) => (
                    <Link
                      key={product.id}
                      to={`/product/${product.sku}`} // Clicking opens the product details
                    ...
                    >
                      <div className="aspect-[4/5] bg-cream relative overflow-hidden">
                        ...
                        />
                        {product.badge && (
                          <div className="absolute top-2 left-2 bg-primary text-white text-[9px] font-medium tracking-widest uppercase px-2 py-0.5 rounded-full">
                            {product.badge}
                          </div>
                        )}
                      </div>
