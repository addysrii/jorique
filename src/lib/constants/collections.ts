...
export const JORIQUE_COLLECTIONS: Record<string, JoriqueCollection> = {
  ...
  luxe: {
    ...
    name: 'JORIQUE Luxe',
    shortName: 'Luxe',
    background: '#0B5F61', // Deep Teal
    primaryText: '#F5EDE3', // Warm Ivory
    secondaryText: 'rgba(245, 237, 227, 0.85)',
    accentColor: '#C6A96B', // Gold
    borderColor: 'rgba(198, 169, 107, 0.45)',
    innerBg: '#08484A',
    cardBg: 'rgba(0, 0, 0, 0.22)',
    badgeBg: '#0B5F61', // Deep Teal
    badgeText: '#F5EDE3', // Warm Ivory
 ...
 * - JORIQUE Luxe: Deep Teal #0B5F61 | Warm Ivory #F5EDE3
...
export function getBadgeColors(badgeText?: string): { bg: string; text: string; border?: string } {
  ...
  if (clean.includes('luxe')) {
    return { bg: '#0B5F61', text: '#F5EDE3', border: 'rgba(245, 237, 227, 0.35)' };
  }
  ...
  if (clean.includes('best') || clean.includes('seller')) {
    return { bg: '#0B5F61', text: '#F5EDE3', border: 'rgba(245, 237, 227, 0.35)' }; // Deep Teal
  }
