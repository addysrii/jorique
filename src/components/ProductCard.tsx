  ...
  return (
    ...
    >
      <Link to={`/product/${product.sku}`} className="group block focus:outline-none">
        <div className="relative overflow-hidden rounded-2xl bg-cream dark:bg-[#1C1A18] aspect-[4/5] border border-border/70 dark:border-[#2E2925] shadow-xs group-hover:shadow-2xl group-hover:shadow-black/10 group-hover:border-primary/30 dark:group-hover:border-[#D4AF37]/40 transition-all duration-400">
          <img
            src={getProductImage()}
            alt={product.name}
            className="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-106 brightness-[0.98] dark:brightness-[0.90]"
          />
          
          {/* Floating Badge with Brand Collection Colors */}
          {product.badge && (() => {
          ...
          <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/85 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0 z-30 flex items-center gap-2">
            <button
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
        ...
        <div className="mt-3.5 px-1">
          <h3 className="font-serif text-base font-semibold text-text dark:text-[#FCFAF7] leading-snug group-hover:text-[#C6A96B] transition-colors duration-200">
            {product.name}
          </h3>
          ...
          {/* Category */}
          {product.category && (
            <p className="mt-0.5 text-xs text-secondary/70 dark:text-white/50 tracking-wide font-sans">
              {product.category}
            </p>
          )}
        </div>
